...
fontpath_s = "fonts/AppleSDGothicNeoB.ttf"
        ...
        if ret is True:
            ...
            if faces is not None:
                for i in range(person_num):
                    ...
                    mask_list.append(mask_on)
                    box_list.append(box)
                    # print('b', box)
                    # int(box[2] - box[0]) 크기 : 145
                    w = int(box[2] - box[0])
                    d = int((box[2] - box[0])/2)
                    ...
                    draw.text((box[0]+font_size/2 , box[1] + font_size/2 ),'%s'%text, font = font_ko_s, fill=color)
                    frame = np.array(frame)
                    box_sizee = (box[2]-box[0]) * (box[3]-box[1])
                    p_size = (box_sizee / (scales[0]*scales[1]))*100
                    # print('d: %d'%d)
                # if (mask_on is False) & (p_size > 0.3):
                if (False in mask_list) & (p_size > 0.3):
                    p_size = 0
                    m_index = mask_list.index(False)
                    # box = box_list[m_index]
                    label = detect.detect_emotion(frame, box, classifier, font)
                    if (count % 5 == 0) or (count == 1):
                        age, g = detect.detect_age_gender_ko(frame, box, model_age, model_gender, align_t)
                    frame = Image.fromarray(frame)
                    draw = ImageDraw.Draw(frame)
                    draw.text((box[0] + 5, box[3] + 10),'%d 세'%(int(age[0])), font = font_ko_l, fill=(0, 0, 0))
                    draw.text((box[0] + 5, box[3] + 10),'%d 세'%(int(age[0])), font = font_ko_s, fill=(255, 255, 255))
                    draw.text((box[0] + font_size*3, box[3] + 10),'%s'%g, font = font_ko_l, fill=(0, 0, 0))
                    draw.text((box[0] + font_size*3, box[3] + 10),'%s'%g, font = font_ko_s, fill=(255, 255, 255))
                    draw.text((box[2] - d/2, box[1] - d/2), '%s'%label, font = font_ko_l, fill=(0, 0, 0))
                    draw.text((box[2] - d/2, box[1] - d/2), '%s'%label, font = font_ko_s, fill=(255, 255, 255))
                    frame = np.array(frame)

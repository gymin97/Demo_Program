            ...
            if faces is not None:
                for i in range(person_num):
                    ...
                    if (mask_on is False) & (p_size > 0.3):
                        ...
                        draw.text((box[2] + 5, box[1] + 10),'%d'%(int(age[0])), font = font_ko_l, fill=(0, 0, 0))
                        draw.text((box[2] + 5, box[1] + 10),'%d'%(int(age[0])), font = font_ko_s, fill=(255, 255, 255))
                        draw.text((box[2] + 5, box[1] + 10 + font_size),'%s'%g, font = font_ko_l, fill=(0, 0, 0))
                        draw.text((box[2] + 5, box[1] + 10 + font_size),'%s'%g, font = font_ko_s, fill=(255, 255, 255))
                        draw.text((box[2] + d/4, box[3] - d/2), label, font = font_ko_l, fill=(0, 0, 0))
                        draw.text((box[2] + d/4, box[3] - d/2), label, font = font_ko_s, fill=(255, 255, 255))
